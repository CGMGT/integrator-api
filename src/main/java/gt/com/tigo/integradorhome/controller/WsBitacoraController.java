...
import gt.com.tigo.integradorhome.service.WsBitacoraService;
...
@CrossOrigin
@RestController
@RequestMapping("/bitacora-ws")
public class WsBitacoraController extends AbstractController<WsBitacoraEntity, WsBitacoraService> {

    private static final Logger LOGGER = LoggerFactory.getLogger(WsBitacoraController.class);
    ...
    @Override
    public ResponseEntity<TigoResponseDto> create(WsBitacoraEntity entity, Long requesterId) {
        return null;
    }

    @Override
    public ResponseEntity<TigoResponseDto> update(WsBitacoraEntity entity, Long requesterId) {
        return null;
    }
    ...
    @Authorized
    @PostMapping("/procesar/{requesterId}/{idBitacora}")
    public ResponseEntity procesar(@PathVariable(required = true) Long requesterId, @PathVariable(required = true) String idBitacora) {
        LOGGER.debug("@{}::procesar({}, {})", this.getClass().getName(), requesterId, idBitacora);

        try {
            return ResponseEntity.ok(new TigoResponseDto(this.service.procesar(requesterId, idBitacora)));
        } catch (Exception ex) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(new TigoResponseDto(HttpStatus.INTERNAL_SERVER_ERROR.value(), ex.getMessage()));
        }
    }
